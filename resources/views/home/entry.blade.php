            ...
            {!! Form::open(['route' => ['entry.confirm', $user->display_url, $job->id], 'class' => 'form-horizontal']) !!}
                ...
                <table class="table-form mb40">
                    <tbody>
                        ...
                        <tr class="form-group @if(!empty($errors->first('carreer'))) has-error @endif">
                            <th>{!! Form::label('carreer', '職務経歴', ['class'=>'control-label']) !!}</th>
                            <td>
                                {!! Form::textarea('carreer', old('carreer'), ['class' => 'form-control form-lg', 'rows' => '4']) !!}
                                <span class="help-block-ent">{{$errors->first('carreer')}}</span>
                            </td>
                        </tr>
                        <tr class="form-group @if(!empty($errors->first('qualification'))) has-error @endif">
                            <th>{!! Form::label('qualification', '保有資格', ['class'=>'control-label']) !!}</th>
                            <td>
                                {!! Form::textarea('qualification', old('qualification'), ['class' => 'form-control form-lg', 'rows' => '3']) !!}
                                <span class="help-block-ent">{{$errors->first('qualification')}}</span>
                            </td>
                        </tr>
                        <tr class="form-group @if(!empty($errors->first('myself'))) has-error @endif">
                            <th><label for="myself" class="control-label">自己PR<span class="fs13 red"> (※)</span></label></th>
                            <td>
                                {!! Form::textarea('myself', old('myself'), ['class' => 'form-control form-lg', 'rows' => '4']) !!}
                                <span class="help-block-ent">{{$errors->first('myself')}}</span>
                            </td>
                        </tr>
